...
function SPViewReturns() {
  ...
  const handleDateChange = (type, value) => {
    ...
    applyFilters(searchQuery, type === "start" ? value : startDate, type === "end" ? value : endDate, minPrice, maxPrice, filterOption);
  };
  ...
  const handlePriceChange = (type, value) => {
    ...
    applyFilters(searchQuery, startDate, endDate, type === "min" ? value : minPrice, type === "max" ? value : maxPrice, filterOption);
  };
  ...
  const generatePDF = () => {
    ...
    filteredReturns.forEach((sale, i) => {
      ...
      doc.text(`Return ID: ${sale.returnId || sale.indirectreturnId}`, 20, yOffset + 7);
      ...
      <div className="spviewreturns-sales-container">
        ...
        <div className="spviewreturns-advanced-filters">
          ...
          <div className="spviewreturns-filter-container">
            ...
            <div className="spviewreturns-filter-group">
              <label>Sort By</label>
              <select className="spviewreturns-filter-select" value={filterOption} onChange={handleFilter}>
              ...
              {currentItems.length > 0 ? (
                currentItems.map((returnData) => (
                  <tr 
                    ...
                    className={hoveredRowId === (returnData.returnId || returnData.indirectreturnId) ? "spviewreturns-hovered-row" : ""}
                    onMouseEnter={() => setHoveredRowId(returnData.returnId || returnData.indirectreturnId)}
                    onMouseLeave={() => setHoveredRowId(null)}
                  >
                    <td>{returnData.returnId || returnData.indirectreturnId}</td>
                    <td>{returnData.buyerId}</td>
                    <td>{new Date(returnData.date).toLocaleString()}</td>
                    <td className="spviewreturns-amount-cell">${returnData.totalAmount.toFixed(2)}</td>
                    <td>{returnData.returnMethod}</td>
                    <td>
                      <button 
                        onClick={() => returnsviewDetails(returnData.returnId || returnData.indirectreturnId)} 
                ...
                <tr>
                  <td colSpan="6" className="spviewreturns-no-results">No returns found</td>
                </tr>
              )}
            </tbody>
          </table>
            ...
            <div className="spviewreturns-page-numbers">
              ...
              {[...Array(Math.min(5, totalPages))].map((_, idx) => {
                ...
                return (
                  <button
                    key={idx}
                    className={`spviewreturns-page-number ${currentPage === pageNum ? 'spviewreturns-active' : ''}`}
              ...
              <div className="spviewreturns-invoice-details">
                <div className="spviewreturns-invoice-header">
                  <div className="spviewreturns-invoice-info">
                    <p><strong>Return ID:</strong> {selectedReturn.returnId || selectedReturn.indirectreturnId}</p>
                  ...
                  <div className="spviewreturns-buyer-info">
                    <p><strong>Buyer ID:</strong> {selectedReturn.buyerId}</p>
                    {selectedReturn.buyerName && <p><strong>Buyer Name:</strong> {selectedReturn.buyerName}</p>}
                    {selectedReturn.buyerEmail && <p><strong>Buyer Email:</strong> {selectedReturn.buyerEmail}</p>}
                    {selectedReturn.buyerPhone && <p><strong>Buyer Phone:</strong> {selectedReturn.buyerPhone}</p>}
                  </div>
                </div>
                ...
                <table className="spviewreturns-items-table">
                  ...
                  <tfoot>
                    <tr>
                      <td colSpan="3" className="spviewreturns-total-label">Total Amount:</td>
                      <td className="spviewreturns-total-amount">${selectedReturn.totalAmount.toFixed(2)}</td>
                    </tr>
                  </tfoot>
                </table>
