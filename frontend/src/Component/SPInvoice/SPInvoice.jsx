...
function SPInvoice() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log("Sending Sale Data:", JSON.stringify(indirectsaleData, null, 2));
      ...
      console.log("Server Response:", JSON.stringify(saleResponse.data, null, 2));
      ...
      if (saleResponse.data.success) {
        ...
        setError("Failed to record sale: " + (saleResponse.data.message || "Unknown error"));
      }
    } catch (error) {
  ...
  return (
    <div className="spinvoice-container">
      ...
      <div id="spinvoice-content">
        ...
        <div className="spinvoice-info-section">
          ...
          <div className="spinvoice-date-details">
            ...
            <p><span>Date:</span> {currentTime.toLocaleDateString()}</p>
            <p><span>Time:</span> {currentTime.toLocaleTimeString()}</p>
          </div>
        </div>
        ...
        <div className="spinvoice-items-section">
          ...
          {invoiceData.map((item, index) => (
            <div key={index} className="spinvoice-item-row">
              ...
              <div className="spinvoice-item-description">{item.selectedItem}</div>
              <div className="spinvoice-item-price">${item.price.toFixed(2)}</div>
              <div className="spinvoice-item-qty">{item.quantity}</div>
              <div className="spinvoice-item-total">${item.total.toFixed(2)}</div>
            </div>
          ))}
        </div>
      ...
      <div className="spinvoice-button-container">
        <button 
          className="spinvoice-back-button" 
          onClick={handleBack}
        >
          Back
        </button>
