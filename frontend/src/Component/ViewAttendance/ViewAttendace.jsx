...
function ViewAttendance() {
  ...
  const fetchAttendance = async () => {
    ...
    try {
      ...
      if (response.data.success && Array.isArray(response.data.data)) {
        ...
        console.log("Attendance Records Set:", response.data.data); // Debug: Confirm data
      } else {
        setError("Invalid attendance data received from server.");
        console.warn("Invalid Response Structure:", response.data); // Debug: Warn on invalid data
      }
    } catch (error) {
      console.error("Error fetching attendance:", error.message); // Debug: Log error details
      ...
      setLoading(false);
      console.log("Loading state set to false"); // Debug: Confirm loading ends
    }
  };
  ...
  const filteredRecords = attendanceRecords.filter((record) => {
    if (!record.date) {
      console.warn("Record missing date:", record); // Debug: Log malformed records
      return false;
    }
