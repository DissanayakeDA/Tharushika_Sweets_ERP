...
function IndirectBuyerSales() {
  ...
  const fetchSalesData = async () => {
    setLoading(true);
    try {
      const response = await axios.get("http://localhost:5000/api/indirectsales");
  ...
  useEffect(() => {
    ...
    if (minPrice) {
      results = results.filter(sale => sale.totalAmount >= parseFloat(minPrice));
    }
    
    if (maxPrice) {
      results = results.filter(sale => sale.totalAmount <= parseFloat(maxPrice));
    }
  ...
  }, [searchQuery, indirectsales, startDate, endDate, minPrice, maxPrice, status, sortField, sortDirection]);

  const viewDetails = async (invoiceId) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/indirectsales/${invoiceId}`);
      if (response.data.success) {
      ...
      <div className="indirectsales-sales-container">
        ...
        <div className="indirectsales-advanced-filters">
          ...
          <div className="indirectsales-filter-container">
            ...
            <button 
              className="indirectsales-reset-btn"
        ...
        <div className="indirectsales-results-summary">
          <div>Showing {indexOfFirstItem + 1}-{Math.min(indexOfLastItem, filteredSales.length)} of {filteredSales.length} results</div>
        ...
        <div className="indirectsales-table-container">
          <table className="indirectsales-sales-table">
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan="5" className="indirectsales-no-results">Loading...</td>
                </tr>
              ) : currentItems.length > 0 ? (
                currentItems.map((sale) => (
                  <tr 
                    key={sale.invoiceId}
                    className={hoveredRow === sale.invoiceId ? "indirectsales-hovered-row" : ""}
                  ...
                  >
                    ...
                    <td className="indirectsales-amount-cell">${sale.totalAmount.toFixed(2)}</td>
                    <td>
                      <button 
                ...
                <tr>
                  <td colSpan="5" className="indirectsales-no-results">No sales found</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="indirectsales-pagination-controls">
            ...
            <div className="indirectsales-page-numbers">
              {getPageNumbers().map((number, index) => (
                number === "..." ? (
                  <span key={`ellipsis-${index}`} className="indirectsales-ellipsis">...</span>
                ) : (
                  <button
                    key={number}
                    className={`indirectsales-page-number ${currentPage === number ? "indirectsales-active" : ""}`}
              ...
              <div className="indirectsales-invoice-details">
                <div className="indirectsales-invoice-header">
                  <div className="indirectsales-invoice-info">
                    ...
                    <p><strong>Status:</strong> {selectedSale.status || "Completed"}</p>
                  </div>
                  
                  <div className="indirectsales-buyer-info">
                    <p><strong>Buyer ID:</strong> {selectedSale.buyerId}</p>
                    {selectedSale.buyerName && (
                      <p><strong>Buyer Name:</strong> {selectedSale.buyerName}</p>
                    )}
                    {selectedSale.buyerContact && (
                      <p><strong>Contact:</strong> {selectedSale.buyerContact}</p>
                    )}
                  </div>
                </div>
                ...
                <table className="indirectsales-items-table">
                  ...
                  <tfoot>
                    <tr>
                      <td colSpan="3" className="indirectsales-total-label">Total Amount:</td>
                      <td className="indirectsales-total-amount">${selectedSale.totalAmount.toFixed(2)}</td>
                    </tr>
                  </tfoot>
                </table>
