...
router.post("/add", async (req, res) => {
  try {
    ...
    console.log("Attempting to save sale with data:", JSON.stringify({
      buyerId,
      items,
      totalAmount,
      date: new Date()
    ...
    try {
      ...
      console.log("Full saved sale document:", JSON.stringify(savedSale, null, 2));
      ...
      console.log("Verified sale in database:", JSON.stringify(verifiedSale, null, 2));
    } catch (saveError) {
      ...
      if (saveError.code === 11000) {
        ...
        indirectsale.invoiceId = `INV-${Date.now()}-${Math.floor(Math.random() * 1000)}`;
...
router.get("/", async (req, res) => {
  try {
    const indirectsales = await indirectSales.find()
      .sort({ date: -1 });
    res.status(200).json({ 
      success: true, 
      sales: indirectsales
