...
router.post("/add", async (req, res) => {
  try {
    ...
    console.log("All stock items in database:", JSON.stringify(allStockItems, null, 2));
    console.log("Available item names:", allStockItems.map(item => item.sp_name));
    ...
    for (const item of items) {
      ...
      if (!stock) {
        const availableItems = allStockItems.map(item => item.sp_name).join(", ");
    ...
    const newReturn = new IndirectReturns({
      ...
      buyerId,
      items,
      totalAmount: returnMethod === "issueMoney" ? totalAmount : calculatedTotalAmount,
      returnMethod,
      date: new Date()
    });

    console.log("Attempting to save new return:", JSON.stringify(newReturn, null, 2));
    ...
    for (const item of items) {
      try {
        ...
        console.log(`Subtracted ${item.quantity} from stock for ${item.itemName}`);
      } catch (stockError) {
...
router.get("/", async (req, res) => {
  try {
    const returns = await IndirectReturns.find()
      .sort({ createdAt: -1 });
    res.json(returns);
  } catch (error) {
...
router.get("/:returnId", async (req, res) => {
  try {
    const returnRecord = await IndirectReturns.findOne({ indirectreturnId: req.params.returnId });
    if (!returnRecord) {
