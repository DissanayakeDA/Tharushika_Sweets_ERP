...
export const addBuyers = async (req, res, next) => {
  const { name, contact, address, country, email, postalCode } = req.body;

  let buyer;

  try {
    if (!name || !contact || !address || !country || !postalCode) {
      return res.status(400).json({ message: "All fields are required" });
    }

    buyer = new Buyer({ name, contact, address, country, email, postalCode });
    ...
    return res
      .status(500)
      .json({ message: "Unable to add buyer due to server error" });
  }
